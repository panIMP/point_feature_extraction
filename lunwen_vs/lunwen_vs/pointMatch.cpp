...
Int32 describe(Uint8* img, Uint8* img_Mark, Feat* zFeat, Int16 width, Int16 height, Int8 window, Int8 orderMax)
{
	Int32       i = 0;
...
getFeat(cv::Mat imgMat, string matName, Feat* zFeat, Int16 width, Int16 height, Int32 tarPointNum, Int8 window, Int8 orderMax)
{
	...
	curPointNum = harris(img, img_Ix_out, img_Iy_out, img_Ixy_out, img_Mark, width, height, tarPointNum);


	// step 4 -- using SURF/Hu/Zernike descriptors to descript the extreme points
	actPointNum = describe(img, img_Mark, zFeat, width, height, window, orderMax);
	...
	/*--------------------- Return Feature -----------------------*/
	cv::imwrite("G:/Pics/Results/" + matName, imgMat);
	return actPointNum;
}
